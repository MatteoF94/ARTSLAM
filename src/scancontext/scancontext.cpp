...
void SCManager::makeAndSaveScancontextAndKeys(const pcl::PointCloud<SCPointType> & _scan_down )
{
    ...
    polarcontexts_.insert(std::make_pair(_scan_down.header.seq, sc));
    polarcontext_invkeys_.insert(std::make_pair(_scan_down.header.seq, ringkey));
    polarcontext_vkeys_.insert(std::make_pair(_scan_down.header.seq, sectorkey));
    polarcontext_invkeys_mat_.push_back( polarcontext_invkey_vec );

    sequence_to_index_[_scan_down.header.seq] = polarcontext_invkeys_mat_.size() - 1;
    index_to_sequence_[polarcontext_invkeys_mat_.size() - 1] = _scan_down.header.seq;
...
std::pair<int, float> SCManager::detectLoopClosureID ( void )
{
    ...
    auto curr_desc = std::prev(polarcontexts_.end())->second; // current observation (query)
...
std::vector<std::pair<int, float>> SCManager::detectLoopClosureID(const std::vector<unsigned int>& candidates, const unsigned int query_idx, unsigned long max_candidates) {
    int loop_id { -1 }; // init with -1, -1 means no loop (== LeGO-LOAM's variable "closestHistoryFrameID")

    std::stringstream msg;

    if(polarcontexts_.find(query_idx) == polarcontexts_.end()) {
        std::cerr << "not existing query inedx\n";
        std::pair<int, float> result {loop_id, 0.0};
        std::vector<std::pair<int, float>> vec = {result};
        return vec; // Early return
    }

    auto curr_key = polarcontext_invkeys_mat_[sequence_to_index_[query_idx]]; // current observation (query)
    auto curr_desc = polarcontexts_[query_idx]; // current observation (query)

    /*
     * step 1: candidates from ringkey tree_
     */
    // tree_ reconstruction (mandatory to make everytime)
    TicToc t_tree_construction;

    polarcontext_invkeys_to_search_.clear();
    for(unsigned int candidate : candidates) {
        if(polarcontexts_.find(candidate) != polarcontexts_.end()) {
            polarcontext_invkeys_to_search_.emplace_back(polarcontext_invkeys_mat_[sequence_to_index_[candidate]]);
        } else {
     ...
     */
    TicToc t_calc_dist;
    std::multimap<double, std::pair<unsigned int, float>> ordered_candidates;
    for (int candidate_iter_idx = 0; candidate_iter_idx < NUM_CANDIDATES_FROM_TREE; candidate_iter_idx++ ) {
        MatrixXd polarcontext_candidate = polarcontexts_[candidates[candidate_indexes[candidate_iter_idx]]];
        std::pair<double, int> sc_dist_result = distanceBtnScanContext( curr_desc, polarcontext_candidate );
        ...
        if(candidate_dist < SC_DIST_THRES) {
            ordered_candidates.insert(std::make_pair(candidate_dist,std::make_pair(candidates[candidate_indexes[candidate_iter_idx]], deg2rad(candidate_align * PC_UNIT_SECTORANGLE))));
            msg << "[Loop found] Nearest distance: " << candidate_dist << " btn " << query_idx << " and " << candidates[candidate_indexes[candidate_iter_idx]] << "." << endl;
        }
    }
    t_calc_dist.toc("Distance calc");
    std::cout << msg.str();
    ...
    unsigned int min_candidates = std::min(max_candidates, ordered_candidates.size());
    for(int i = 0; i < min_candidates; i++) {
        auto it = ordered_candidates.begin();
        std::advance(it, i);
