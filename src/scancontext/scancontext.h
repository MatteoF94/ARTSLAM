...
public:
    ...
    std::vector<std::pair<int, float>> detectLoopClosureID(const std::vector<unsigned int>& candidates, unsigned int query_idx, unsigned long max_candidates); // int: nearest node index, float: relative yaw
...
public:
    ...
    const double SC_DIST_THRES = 0.5; // empirically 0.1-0.2 is fine (rare false-alarms) for 20x60 polar context (but for 0.15 <, DCS or ICP fit score check (e.g., in LeGO-LOAM) should be required for robustness)
    ...
    std::map<unsigned int, Eigen::MatrixXd> polarcontexts_;
    std::map<unsigned int, Eigen::MatrixXd> polarcontext_invkeys_;
    std::map<unsigned int, Eigen::MatrixXd> polarcontext_vkeys_;

    KeyMat polarcontext_invkeys_mat_;
    KeyMat polarcontext_invkeys_to_search_;
    std::unique_ptr<InvKeyTree> polarcontext_tree_;

    std::unordered_map<unsigned int, unsigned int> sequence_to_index_;
    std::unordered_map<unsigned int, unsigned int> index_to_sequence_;
